    ...
    start(options) {
        return new Promise((resolve, reject) => {
            ...
            if (options.rateLimit) {
                ...
                console.log('[RateLimit] redis config:', JSON.stringify({
                    redis: options.rateLimit.redis,
                    enabled: options.rateLimit.redis && options.rateLimit.redis.enabled,
                    hasClient: !!(options.rateLimit.redis && options.rateLimit.redis.client)
                }));
                if (options.rateLimit.redis && options.rateLimit.redis.enabled && options.rateLimit.redis.client) {
                    // eslint-disable-next-line no-console
                    console.log('[RateLimit] Using Redis store');
                    rateLimitConfig.store = new RedisStore({
                        client: options.rateLimit.redis.client,
                        expiry: options.rateLimit.ms / 1000
                    });
                }
                else {
                    // eslint-disable-next-line no-console
            ...
            if (options.logger) {
                this._app.use(logger(options.logger));
            }
